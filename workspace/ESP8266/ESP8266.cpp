...
String ESP8266::getLocalIP(void) {
	...
	while (m_puart->available() > 0) {
		...
		if (inData.indexOf("IP") != -1) {
			delay(100);
			return ("IP: "
					+ inData.substring(inData.indexOf("IP") + 4,
							inData.length() - 2));
		}
	}
...
int ESP8266::httpGet2(String webserver, uint32_t port, char* request) {

	if (createTCP(webserver, port)) {
		Serial.println(F("create tcp - OK"));
	} else {
		Serial.println(F("create tcp - ERROR"));
		return -1;
...
char* ESP8266::httpGet3(String webserver, uint32_t port, char* request) {
	...
	int len = recvSingle(m_responseBuffer, MAX_BUFFER_SIZE);
	char* response = (char*) m_responseBuffer;
	Serial.println(response);

	return response;
}

int ESP8266::getCrvena(char* response) {
	int i = 0;
	int crvena, zuta;
	while (response[i] != '\0') {
		if ('#' == response[i]) {
			return response[i + 1] - '0';
		}
		i++;
	}
	return 0;
...
int ESP8266::getZuta(char* response) {
	int i = 0;
	int crvena, zuta;
	while (response[i] != '\0') {
		if ('#' == response[i]) {
			return zuta = response[i + 3] - '0';
		}
		i++;
	}
	return 0;
